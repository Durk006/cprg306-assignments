  ...
  return (
    <main>
        <div className="mt-16 text-stone-50">
            <p className="text-stone-50 text-xl font-bold">Meal Ideas</p>
          
            <ul>
                {meals === null ?  <li>{info}</li> :
                meals.map((meal) => (
                
                <li className="bg-slate-800 rounded mt-2 p-1.5 w-80 hover:bg-orange-800 cursor-pointer" key={meal.idMeal} onClick={() => onMealSelect(meal.idMeal)}>
                    <h2>{meal.strMeal}</h2>
                    {mealIngredients != null &&
                    meal.idMeal === mealIngredients[0].idMeal &&
                    mealIngredients.map((mealIng) => (
                      <li className key={mealIng.idMeal}>
                        <h3 className="text-gray-400 text-xs">Ingredients needed:</h3>
                        <ul className="ml-6 text-gray-400 text-xs">
