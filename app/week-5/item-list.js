...
export default function ItemList() {
  ...
  return (
    <div>
      <div className="p-2 m-2 text-l capitalize text-stone-50 "> Sort By: 
        <button className={`p-1 m-2 w-28 square-lg  ${sortBy === 'name' ? 'bg-orange-500' : 'bg-orange-700'}`} onClick={() => setSortBy('name')}>Name</button>
        <button className={`p-1 m-2 w-28 square-lg  ${sortBy === 'category' ? 'bg-orange-500' : 'bg-orange-700'}`} onClick={() => setSortBy('category')}>Category</button>
        <button className={`p-1 m-2 w-32 square-lg  ${sortBy === 'group' ? 'bg-orange-500' : 'bg-orange-700'}`} onClick={() => setSortBy('group')}>Grouped Category</button>
      </div>
      ...
      {sortBy === 'group' ? (
        ...
        Object.keys(groupedItems).sort().map((category) => (
          <div  key={category}>
            <h1 className="text-lg ml-4 capitalize text-white ">{category}</h1>
            <ul>
